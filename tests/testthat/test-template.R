...
#    `functionName` is a function's name in your module (e.g., `priorityPlaces_DataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/GitHub/priorityPlaces_DataPrep/priorityPlaces_DataPrep/priorityPlaces_DataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("priorityPlaces_DataPrep")
  path <- list(modulePath = "C:/Users/tmichele/GitHub/priorityPlaces_DataPrep/priorityPlaces_DataPrep",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    priorityPlaces_DataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("priorityPlaces_DataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- priorityPlaces_DataPrepEvent1(mySim)
  } else {
  ...
  if (exists("priorityPlaces_DataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- priorityPlaces_DataPrepEvent2(mySim)
  } else {
